...
Item {
    ...
    function call(method, params, done) {
        if (!socket.connected) {
            if (done)
                    ...
                    "message": "Rayarchy daemon is not running"
                });
            return;
        }
        var id = rpc.nextId++;
    ...
    function handle(line) {
        ...
        if (obj.id !== undefined && rpc.pending[obj.id]) {
            var done = rpc.pending[obj.id];
            delete rpc.pending[obj.id];
            done(obj.result || null, obj.error || null);
            return;
        }
